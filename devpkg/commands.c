...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only)
{
	...
	if(apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS) {
		...
		check(!fetch_only, "No point in getching a DEPENDS file");
		...
		sentinel("Don't know how to handle that: %s", url);
	}
...
int Command_build(apr_pool_t *p, const char *url, const char *configure_opts,
	...
	rc = DB_update(url);
	check(rc == 0, "Failed to add this package to the databse.");
